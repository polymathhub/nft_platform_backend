"""
Comprehensive fix for alembic_version column size issue.

This script:
1. Downgrades the database to before migration 011 (which has the long name)
2. Expands the alembic_version.version_num column to VARCHAR(128)
3. Re-runs all migrations including the new migration 012 that documents the fix
"""
...
async def main():
    ...
    # First, try to fix the table directly
