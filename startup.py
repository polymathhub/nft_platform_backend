...
Handles:
1. PostgreSQL async connection with asyncpg
2. Idempotent enum type creation
3. Alembic migration execution
4. FastAPI app startup with uvicorn
"""
...
# Enum types used in the project
ENUM_TYPES = {
    "notificationtype": [
        "PAYMENT_RECEIVED",
        "PAYMENT_FAILED",
...
async def ensure_enum_types(database_url: str) -> bool:
    """
    Ensure all required enum types exist in PostgreSQL.
    Creates missing enum types safely.
    
    Args:
        database_url: PostgreSQL async database URL
        
    Returns:
        True if all enums are ready, False if errors occurred
    """
    engine = create_async_engine(
        database_url,
        echo=False,
...
async def main() -> int:
    ...
    # Step 2: Ensure enum types exist
